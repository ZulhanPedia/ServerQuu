...
app.post('/api/upload/init', async (req, res) => {
    const { name, size, mimeType, targetEmail, folderId } = req.body;
...
app.post('/api/upload/confirm', async (req, res) => {
    const { name, size, mimeType, accountEmail, googleDriveFileId, folderId, folderName } = req.body;
    const db = readDb();
    const accounts = readAccountsConfig();
    ...
    if (!isSimulatedAccount(targetAccount)) {
        try {
            ...
            // Log kesalahan ke konsol dan logs virtual, namun biarkan kode tetap berjalan menyimpan berkas
...
app.post('/api/upload/local', upload.single('file'), (req, res) => {
    ...
    addLog(`Berkas "${req.file.originalname}" disimpan secara lokal di [${email}]/` + (targetFolderName ? `${targetFolderName}/` : ''));
    res.json({ success: true });
});
...
<body class="text-slate-800 antialiased overflow-hidden flex flex-col" style="height: 100vh; height: 100dvh; background-color: #fcfcfd;">
    ...
    <!-- MAIN PANEL (DIBUAT SEBAGAI FULL HEIGHT VIEWPORT FLEXBOX) -->
    <div id="mainDashboard" class="hidden h-full flex flex-col overflow-hidden">
        
        <!-- HEADER (DIPAKSA FIXED DI BAGIAN PALING ATAS - SHRINK-0) -->
        <header class="bg-white border-b border-slate-100 shrink-0 z-40">
            <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
                ...
                <!-- SISI KANAN HEADER: Layout Sejajar Author Badge di Sebelah Kiri Kunci Panel -->
        ...
        <!-- KONTEN UTAMA (TERBAGI MENJADI NON-SCROLLABLE ATAS & SCROLLABLE TENGAH) -->
        <main class="flex-grow flex flex-col overflow-hidden max-w-6xl w-full mx-auto px-4 pt-4 pb-14">
            
            <!-- SECTION FIXED TOP (TOTAL STORAGE + NAVIGASI TAB DIKUNCI TIDAK BERUBAH - SHRINK-0) -->
            <div class="shrink-0 space-y-4 mb-4">
                
                <!-- CARD TOTAL STORAGE GABUNGAN -->
                ...
                <!-- TAB NAVIGASI UTAMA (TETAP DI POSISI ATAS) -->
            ...
            <!-- KARTU AKTIF KONTEN (HANYA BAGIAN INI YANG BISA DI-SCROLL SECARA VERTIKAL - FLEX-GROW + OVERFLOW-Y-AUTO) -->
            <div id="tabContentContainer" class="flex-grow overflow-y-auto bg-white border border-slate-100 rounded-3xl p-5 sm:p-8 custom-shadow scrollbar-none mb-2">
                ...
                <!-- TAB EXPLORER (DENGAN BAR PENCARIAN & FILTER KATEGORI) -->
                <div id="tab-explorer" class="tab-content hidden">
                    ...
                    <!-- Tombol Filter Kategori File Cepat -->
        ...
        <!-- FOOTER BRANDING BOTTOM (FIXED PERMANEN DI BAGIAN PALING BAWAH - STANDBY POSISI) -->
    ...
    <!-- INTERFACE LOGIC SCRIPT (DENGAN ESCAPE STRING KHUSUS BAGI NODE.JS TEMPLATE LITERAL) -->
    <script>
        ...
        function switchTab(tabId) {
            ...
            if (tabId === 'tab-upload') syncUploadFormAccounts(); // Sync upload dropdown
        ...
        function syncUploadFormAccounts() {
            ...
            // Amankan status email terpilih saat ini sebelum dropdown dirender ulang
            ...
            // Terapkan kembali posisi email manual yang sedang aktif
        ...
        // --- OPTIMALISASI: ASYNC TIMEOUT AGAR PERAMBAN TIDAK LAG SAAT MEMILIH FILE RAKSASA (4GB+) ---
        function updateFileInfo() {
            ...
            if (input.files && input.files[0]) {
                ...
                // Tampilkan loading visual tercepat di UI terlebih dahulu
                document.getElementById("fileSelectLabel").innerText = "Membaca berkas... ⏱️";
                statusText.innerText = "Mendapatkan info metadata...";

                // Delay pemrosesan berat 100ms agar browser mobile menyelesaikan penyerahan file OS secara asinkron
                setTimeout(() => {
        ...
        // --- SISTEM UNGGULAN: GOOGLE DRIVE RESUMABLE RAW STREAM UPLOAD (BEBAS MEMORY OVERHEAD) ---
        function handleUpload(e) {
            ...
            // Langkah 1: Minta otorisasi & link sesi upload langsung dari server Termux
            ...
            .then(data => {
                ...
                // KASUS B: JALUR INSTAN (Google Drive Resumable API)
                // Browser langsung mengirimkan file biner mentah (PUT Stream) ke URL Google tanpa memakan RAM tambahan!
                const xhr = new XMLHttpRequest();
                window.currentXhr = xhr;
                ...
                xhr.addEventListener('load', () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        // Konfirmasi dan verifikasi status upload aman ke Server lokal
                ...
                // Lakukan unggahan langsung PUT STREAM raw file raksasa ke link sesi Google Drive
        ...
        // --- SISTEM DETEKSI DAN NAVIGASI GESER (SWIPE GESTURE) FACEBOOK STYLE ---
        let touchStartX = 0;
        let touchStartY = 0;
        let touchEndX = 0;
        let touchEndY = 0;
