    ...
    <div id="mainDashboard" class="hidden flex-grow pb-12">
        ...
        <main class="max-w-6xl mx-auto px-4 mt-8">
            ...
            <nav class="flex overflow-x-auto gap-2 p-1 bg-slate-100 rounded-2xl mb-8 border scrollbar-none">
                ...
                <button onclick="switchTab('tab-settings')" id="btn-tab-settings" class="tab-btn px-5 py-3 rounded-xl font-medium text-sm transition-all text-slate-600 hover:bg-white/50 whitespace-nowrap">⚙️ Pengaturan PIN</button>
            </nav>
            ...
            <div class="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 custom-shadow">
                ...
                <div id="tab-accounts" class="tab-content">
                    <div class="mb-6 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-slate-500 text-xs">Dikelola otomatis via file <code class="bg-slate-100 text-sky-600 font-bold px-1.5 py-0.5 rounded">config.json</code>.</p>
                        </div>
                ...
                <div id="tab-settings" class="tab-content hidden">
                    <h3 class="text-lg font-bold text-slate-900 mb-1">Ganti PIN Keamanan</h3>
                    <p class="text-slate-500 text-xs mb-6">Silakan perbarui PIN demi menjaga privasi dan enkripsi panel.</p>
                    <form onsubmit="handlePINChange(event)" class="max-w-md space-y-4">
                        <input type="password" id="oldPinInput" placeholder="PIN Lama" class="w-full py-2.5 px-4 border rounded-xl focus:outline-none focus:border-sky-600 text-sm" required>
                        <input type="password" id="newPinInput" placeholder="PIN Baru (Hanya Angka)" class="w-full py-2.5 px-4 border rounded-xl focus:outline-none focus:border-sky-600 text-sm" required>
                        <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold px-5 py-3 rounded-xl transition-all">Simpan PIN Baru</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        function fetchFiles() {
            fetch('/api/files').then(r => r.json()).then(fs => {
                ...
                fs.forEach(f => {
                    // PENCEGAHAN LATEX CRASH: Lakukan kalkulasi di luar dan gabungkan menggunakan string concatenation murni
